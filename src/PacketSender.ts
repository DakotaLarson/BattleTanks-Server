...
export const sendPlayerAddition = (id: number, pos: Vector4, color: number, modelId: string, modelColors: string[], headOffset: number) => {
    const dataObj = {
        id,
        modelId,
        modelColors,
...
export const sendLobbyCode = (id: number, code: string) => {
    const data = constructData(Packet.LOBBY_CODE, code, DataType.STRING);
    send(id, data);
};
